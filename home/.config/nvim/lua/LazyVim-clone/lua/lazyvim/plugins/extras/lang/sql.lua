...
return {
...

  {
    ...
    keys = {
      ...
      { "<leader>ed", "<cmd>DBUIToggle<CR>", desc = "Toggle [d]BUI" },
      -- JH MODIFICATION END
    },
    init = function()
      ...
      -- keymap: <localleader>e (see below / after/ftplugin/sql.lua)
      vim.g.db_ui_execute_on_save = false

      -- JH MODIFICATION START
      -- dadbod-ui's default SQL-buffer maps are <Leader>S/W/E (set buffer-locally
      -- via the plugin's ftplugin/sql.vim). <Leader>S now conflicts with the
      -- scratch-buffer prefix, so disable those defaults and re-bind under
      -- <localleader> in after/ftplugin/sql.lua. Only the SQL-buffer maps are
      -- disabled; the dbui drawer (single letters) and dbout (<Leader>R) maps
      -- don't collide and are left intact.
