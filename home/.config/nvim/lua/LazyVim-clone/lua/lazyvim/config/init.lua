...
function M.load(name)
  local function _load(mod)
    -- LazyVim ejected - check if module exists, don't rely solely on lazy.nvim cache
    local found = require("lazy.core.cache").find(mod)[1] or package.searchpath(mod, package.path)
    if found then
      LazyVim.try(function()
        require(mod)
...
function M.init()
  ...
  M.did_init = true
  -- LazyVim ejected - rtp already set up in lazy.lua
