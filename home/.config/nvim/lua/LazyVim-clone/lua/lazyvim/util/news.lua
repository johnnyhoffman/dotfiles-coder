...
function M.changelog()
  -- LazyVim ejected - open from clone directory
  local clone_dir = vim.fn.stdpath("config") .. "/lua/LazyVim-clone"
  M.open(clone_dir .. "/CHANGELOG.md")
end

function M.lazyvim(when_changed)
  -- LazyVim ejected - open from clone directory
  local clone_dir = vim.fn.stdpath("config") .. "/lua/LazyVim-clone"
  M.open(clone_dir .. "/NEWS.md", { when_changed = when_changed })
end
